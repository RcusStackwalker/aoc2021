...
impl<'a> Board {
    pub fn new<I>(mut it: I) -> Board where I: Iterator<Item=&'a str> {
...
impl Game {
    pub fn new(draws: Draws, boards: Vec<Board>) -> Game {
        ...
        boards.iter().enumerate().for_each(|(board_index, b)| {
            for y in 0..BOARD_SIZE {
                for x in 0..BOARD_SIZE {
                    index.entry(b.rows[y][x].0).or_insert(Vec::new()).push((board_index, x, y));
                }
            }
        });
        Game {
...
fn read_file_into_game(path: &str) -> Game {
    ...
    let draws = draws_string.split(',').map(|s| s.parse::<usize>().unwrap()).collect_vec();
    let boards = it.map(|board_input| {
        Board::new(board_input.lines())
    }).collect_vec();
